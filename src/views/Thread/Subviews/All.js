...
define(function(require, exports, module) {
    ...
    var tpl_thread_quick                 = require('text!./tpl/ThreadQuick.html');
    var template_thread_quick            = Handlebars.compile(tpl_thread_quick);
    ...
    function SubView(options) {
        ...
        window.theContentLayout = this.contentLayout;
    ...
    SubView.prototype.loadModels = function(){
        ...
        this.collection = new ThreadModel.ThreadCollection([],this.options.search_options);
    ...
    SubView.prototype.createDefaultSurfaces = function(){
        ...
        // Bg Surface (behind scrollview)
        this.scrollviewBg = new Surface({
            size: [undefined, undefined],
        });
        this.scrollviewBg.pipe(this.contentLayout);
        this.add(Utils.usePlane('background',1)).add(this.scrollviewBg);
    ...
    SubView.prototype.addOne = function(Thread){
        var that = this;

        var buildThread = function(Thread){

            return template_thread_quick({
        ...
        threadView.Layout.on('click', function(){
            App.history.navigate('thread/' + Thread.get('_id'));
        });
        threadView.Layout.pipe(this.contentLayout);

        // Get related Emails for Thread
        Thread._related.Email.populated().then(function(){

            threadView.Layout.setContent(buildThread(Thread));

        });
        Thread.fetchRelated();    

        // threadView.Layout.sequential.MyVoting.show(threadView.Layout.sequential.MyVoting.VotingResults);

        threadView.Layout.Model = Thread;
    

        this.contentLayout.Views.splice(this.contentLayout.Views.length-1, 0, threadView.Layout);
